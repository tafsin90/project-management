...
function MyTasksSidebar() {
    ...
    const myTasks = currentWorkspace?.projects?.flatMap((project) => project.tasks.filter((task) => task?.assignee?.id === user.id)) || [];
                    ...
                    <div className="space-y-1">
                        {myTasks.length === 0 ? (
                            ...
                            myTasks.map((task, index) => (
                                <Link key={index} href={`/tasks/${task.projectId}/${task.id}`} className="w-full rounded-lg transition-all duration-200 text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-black dark:hover:text-white" >
