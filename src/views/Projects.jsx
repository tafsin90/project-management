...
export default function Projects() {
    ...
    const createWorkspace = async () => {
        ...
        try {
            ...
            toast.success("Workspace created. You can now create a project.");
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                ...
                <button onClick={() => setIsDialogOpen(true)} disabled={!currentWorkspace} title={!currentWorkspace ? "Create a workspace first" : undefined} className="flex items-center px-5 py-2 text-sm rounded bg-gradient-to-br from-blue-500 to-blue-600 text-white hover:opacity-90 transition disabled:cursor-not-allowed disabled:opacity-50" >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProjects.length === 0 ? (
                    <div className="col-span-full text-center py-16">
                        ...
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{currentWorkspace ? "No projects found" : "Create a workspace first"}</h3>
                        <p className="text-gray-500 dark:text-zinc-400 mb-6 text-sm">
                            {currentWorkspace ? "Create your first project to get started" : "A workspace is required before projects can be created."}
                        </p>
                        <button onClick={currentWorkspace ? () => setIsDialogOpen(true) : createWorkspace} className="flex items-center gap-1.5 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded mx-auto text-sm" >
                            <Plus className="size-4" />
                            {currentWorkspace ? "Create Project" : "Create Workspace"}
                        </button>
                    </div>
                ) : (
