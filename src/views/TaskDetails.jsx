...
const TaskDetails = ({ projectId, taskId }) => {
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const workspace = await api.addComment(currentWorkspace.id, projectId, taskId, { content: newComment, userName: user?.name, userEmail: user?.email });
                                ...
                                {comments.map((comment) => (
                                    <div key={comment.id} className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${comment.user.id === user?.id ? "ml-auto" : "mr-auto"}`} >
                                        <div className="flex items-center gap-2 mb-1 text-sm text-gray-500 dark:text-zinc-400">
                                            <Image src={comment.user.image || assets.profile_img_a} alt="avatar" width={20} height={20} className="size-5 rounded-full" />
